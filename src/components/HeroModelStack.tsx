...
export default function HeroModelStack() {
  return (
    <div className="relative w-full h-[380px]">
      ...
      <div
        className="hidden md:block absolute left-[2%] top-[18%] px-4 py-2 bg-black/90 border border-green-400/50 rounded-lg transition-all hover:scale-105 float-slow shadow-lg shadow-green-500/20 cursor-help"
        title="200 Transactions Per Second – High throughput for enterprise workloads"
      >
        <span className="block text-sm text-green-400 font-mono font-semibold">200 TPS</span>
        <span className="block text-xs text-white/80">High throughput</span>
      </div>
      <div
        className="hidden md:block absolute right-[2%] top-[38%] px-4 py-2 bg-black/90 border border-green-400/50 rounded-lg transition-all hover:scale-105 float-slow float-delay-1 shadow-lg shadow-green-500/20 cursor-help"
        title="12 Milliseconds Time To First Token – Ultra-fast response latency"
      >
        <span className="block text-sm text-green-400 font-mono font-semibold">12MS TTFT</span>
        <span className="block text-xs text-white/80">Fast response</span>
      </div>
      <div
        className="hidden md:block absolute left-[2%] top-[55%] px-4 py-2 bg-black/90 border border-green-400/50 rounded-lg transition-all hover:scale-105 float-slow float-delay-2 shadow-lg shadow-green-500/20 cursor-help"
        title="110 Requests Per Minute – Scalable capacity for your applications"
      >
        <span className="block text-sm text-green-400 font-mono font-semibold">110 REQ/M</span>
        <span className="block text-xs text-white/80">Scalable capacity</span>
      </div>

      {/* Floating AI brand logos - hidden on mobile to avoid overlap */}
      {/* OpenAI */}
      <div className="hidden md:flex absolute left-[5%] top-[8%] w-14 h-14 rounded-full bg-black items-center justify-center shadow-xl shadow-black/50 float-animation transition-transform hover:scale-110 border-2 border-white/20">
      ...
      {/* Anthropic Claude */}
      <div className="hidden md:flex absolute right-[5%] top-[50%] w-14 h-14 rounded-full bg-[#D4A27F] items-center justify-center shadow-xl shadow-[#D4A27F]/50 float-animation float-delay-2 transition-transform hover:scale-110 border-2 border-white/20">
        <svg className="w-8 h-8 text-white" viewBox="0 0 24 24" fill="currentColor">
      ...
      {/* Hugging Face */}
      <div className="hidden md:block absolute left-[5%] bottom-[18%] w-14 h-14 rounded-full overflow-hidden shadow-xl shadow-[#FFD21E]/50 float-animation float-delay-1 transition-transform hover:scale-110 border-2 border-white/20">
        <img src="/logos/hf-logo.png" alt="Hugging Face" className="w-full h-full object-cover" />
      </div>

      {/* Google Gemini */}
      <div className="hidden md:block absolute right-[5%] bottom-[30%] w-14 h-14 rounded-full overflow-hidden shadow-xl float-animation float-delay-3 transition-transform hover:scale-110 bg-white p-1 border-2 border-white/20">
        <img src="/logos/google-gemini-icon.webp" alt="Google Gemini" className="w-full h-full object-contain" />
      </div>

      {/* Meta AI */}
      <div className="hidden md:block absolute right-[5%] top-[15%] w-14 h-14 rounded-full overflow-hidden shadow-xl shadow-primary-500/50 float-animation float-delay-2 transition-transform hover:scale-110 border-2 border-white/20">
        <img src="/logos/Meta-ai-logo.webp" alt="Meta AI" className="w-full h-full object-cover" />
      </div>

      {/* Mistral AI */}
      <div className="hidden md:block absolute left-[5%] top-[35%] w-14 h-14 rounded-full overflow-hidden shadow-xl shadow-orange-500/50 float-animation float-delay-3 transition-transform hover:scale-110 bg-black p-2 border-2 border-white/20">
