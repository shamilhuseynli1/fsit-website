...
export default function HeroModelStack() {
  return (
    <div className="relative w-full h-[380px]">
      ...
      <div className="absolute left-[8%] top-[18%] px-3 md:px-4 py-2 bg-black/90 border border-green-400/50 rounded-lg transition-all hover:scale-105 float-slow shadow-lg shadow-green-500/20">
        <span className="block text-xs md:text-sm text-green-400 font-mono font-semibold">200 TPS</span>
        <span className="block text-[10px] md:text-xs text-white/90 font-medium">Tokens Per Second</span>
        <span className="block text-[10px] md:text-xs text-white/70">High throughput</span>
      </div>
      <div className="absolute right-[8%] top-[38%] px-3 md:px-4 py-2 bg-black/90 border border-green-400/50 rounded-lg transition-all hover:scale-105 float-slow float-delay-1 shadow-lg shadow-green-500/20">
        <span className="block text-xs md:text-sm text-green-400 font-mono font-semibold">12MS TTFT</span>
        <span className="block text-[10px] md:text-xs text-white/90 font-medium">Time To First Token</span>
        <span className="block text-[10px] md:text-xs text-white/70">Fast response</span>
      </div>
      <div className="absolute left-[8%] top-[55%] px-3 md:px-4 py-2 bg-black/90 border border-green-400/50 rounded-lg transition-all hover:scale-105 float-slow float-delay-2 shadow-lg shadow-green-500/20">
        <span className="block text-xs md:text-sm text-green-400 font-mono font-semibold">110 REQ/M</span>
        <span className="block text-[10px] md:text-xs text-white/90 font-medium">Requests Per Minute</span>
        <span className="block text-[10px] md:text-xs text-white/70">Scalable capacity</span>
      </div>

      {/* Floating AI brand logos - positioned close to content */}
      {/* OpenAI - top left area */}
      <div className="hidden md:flex absolute left-[3%] top-[5%] w-14 h-14 rounded-full bg-black items-center justify-center shadow-xl shadow-black/50 float-animation transition-transform hover:scale-110 border-2 border-white/20">
      ...
      {/* Anthropic Claude - bottom right area */}
      <div className="hidden md:flex absolute right-[3%] top-[62%] w-14 h-14 rounded-full bg-[#D4A27F] items-center justify-center shadow-xl shadow-[#D4A27F]/50 float-animation float-delay-2 transition-transform hover:scale-110 border-2 border-white/20">
        <svg className="w-8 h-8 text-white" viewBox="0 0 24 24" fill="currentColor">
      ...
      {/* Hugging Face - bottom left area */}
      <div className="hidden md:block absolute left-[3%] bottom-[8%] w-14 h-14 rounded-full overflow-hidden shadow-xl shadow-[#FFD21E]/50 float-animation float-delay-1 transition-transform hover:scale-110 border-2 border-white/20">
        <img src="/logos/hf-logo.png" alt="Hugging Face" className="w-full h-full object-cover" />
      </div>

      {/* Google Gemini - bottom right area */}
      <div className="hidden md:block absolute right-[3%] bottom-[22%] w-14 h-14 rounded-full overflow-hidden shadow-xl float-animation float-delay-3 transition-transform hover:scale-110 bg-white p-1 border-2 border-white/20">
        <img src="/logos/google-gemini-icon.webp" alt="Google Gemini" className="w-full h-full object-contain" />
      </div>

      {/* Meta AI - top right area */}
      <div className="hidden md:block absolute right-[3%] top-[8%] w-14 h-14 rounded-full overflow-hidden shadow-xl shadow-primary-500/50 float-animation float-delay-2 transition-transform hover:scale-110 border-2 border-white/20">
        <img src="/logos/Meta-ai-logo.webp" alt="Meta AI" className="w-full h-full object-cover" />
      </div>

      {/* Mistral AI - left middle area */}
      <div className="hidden md:block absolute left-[3%] top-[42%] w-14 h-14 rounded-full overflow-hidden shadow-xl shadow-orange-500/50 float-animation float-delay-3 transition-transform hover:scale-110 bg-black p-2 border-2 border-white/20">
